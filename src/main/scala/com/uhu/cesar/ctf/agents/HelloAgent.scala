...
class HelloAgent extends Agent with DFRegister with ServerConnectionService {
  ...
  override def setup(): Unit = {
    ...
    addBehaviour(getConnectionBehaviour(
      ...
      (result, msg) => {
        ...
        val behaviourList = List(receiveMessages, randomAction, sendAction)
  ...
  def receiveMessages: BehaviourFunction[HelloAgent, CTFGameData, AgentAction] = { agent =>
    (gd, aa) =>
  ...
  def randomAction: BehaviourFunction[HelloAgent, CTFGameData, AgentAction] = { _ =>
    (gd, aa) =>
      (gd, AgentAction.randomAction)
  }

  def sendAction: BehaviourFunction[HelloAgent, CTFGameData, AgentAction] = { agent =>
    (gd, aa) =>
  ...
  def takeTheFlag: BehaviourFunction[HelloAgent, CTFGameData, AgentAction] = { agent =>
    (data, aa) =>
      ...
      val nextAction :: newActions = if (data.computePath) {
        GoTo(from, Point(enemyFlag.x, enemyFlag.y))
